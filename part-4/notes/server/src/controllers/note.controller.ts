import { CreateNote, UpdateNote } from "@/schemas/note.schema";
import {
  createNote,
  deleteNote,
  findAllNotes,
...
export const handleCreateNote = async (
  req: Request<unknown, unknown, CreateNote["body"]>,
  res: Response
) => {
  ...
  await createNote({ userId, content, important });

  res.status(201).json({ message: "Note created" });
};

export const handleUpdateNote = async (
  req: Request<UpdateNote["params"], unknown, UpdateNote["body"]>,
  res: Response
) => {
  const { id } = req.params;
